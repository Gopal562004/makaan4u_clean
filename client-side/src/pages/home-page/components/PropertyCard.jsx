...
const PropertyCard = memo(
  ({
    ...
    const agentName = property.agentInfo?.name || "A";
    ...
    const showFallbackAvatar =
      !property.agentInfo?.avatar || avatarError === true;
      ...
      >
        ...
        <div className="relative h-48 bg-slate-100 overflow-hidden">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1">
            ...
            <div className="bg-white/95 px-2 py-1 rounded-full text-xs text-slate-700">
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between mt-4 p-3 bg-slate-50 rounded-lg text-xs text-slate-600">
            <div className="flex flex-col items-center">
              ...
              <span className="text-slate-500">{t.bedrooms}</span>
            </div>

            <div className="flex flex-col items-center">
              <Bath className="w-3 h-3 mb-1" />
              {property.specifications?.bathrooms}
              <span className="text-slate-500">{t.bathrooms}</span>
            </div>

            <div className="flex flex-col items-center">
              <Square className="w-3 h-3 mb-1" />
              {property.specifications?.area}
              <span className="text-slate-500">{t.sqft}</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-4 bg-slate-50 p-3 rounded-lg">
            <div className="flex items-center min-w-0">
              {/* Fallback Avatar */}
              {showFallbackAvatar ? (
                <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-xs mr-2">
                  {agentInitial}
                </div>
              ) : (
                <img
                  src={property.agentInfo?.avatar}
